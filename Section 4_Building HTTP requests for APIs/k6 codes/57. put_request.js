...
export default function() {
    group("PUT() method is testing", function(){
        ...
        const put_method_data = JSON.stringify({
            name: "Shit_head_big_put",
            sex: "M",
            //INFO[0004] res2.response:  {"date_of_birth":["Date has wrong format. Use one of these formats instead: YYYY-MM-DD."]}  source=console    
            date_of_birth: "2222-02-03"
        });

        const res3 = http.put(`https://test-api.k6.io/my/crocodiles/${newCrocodileId}/`, put_method_data, params2);
        console.log('res3: ', res3.json());
        console.log('res3.status: ', res3.status);

        check(res3, {
            "status code of assertion res3 and information is putted": (r) => r.status === 200 || r.status === 201,
            "crocodile id is valid": (r) => newCrocodileId === r.json().id,
            "corcodile name is Shit_head_big_modified and this information is updated": (r) => r.json().name === 'Shit_head_big_put',
            "crocodile sex is M and this information is updated": (r) => r.json().sex === 'M'
        })
